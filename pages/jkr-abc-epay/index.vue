  ...
  <view class="container">
    ...
    <view class="section">
      ...
      <view class="intro-card">
        ...
        <text class="intro-desc">
          农行掌银支付鸿蒙SDK封装插件，支持农行掌银支付功能。 通过封装 ABCEPay.har SDK，提供简洁的
          API 供 uni-app 调用。
        </text>
    ...
    <view class="section">
      ...
      <view class="demo-card">
        ...
        <view class="demo-result" v-if="installResult !== null">
          <u-icon
            :name="installResult ? 'checkmark-circle' : 'close-circle'"
            :size="40"
    ...
    <view class="section">
      ...
      <view class="code-card">
        ...
        <view class="code-block">
          <text class="code-text"
            >// #ifdef APP-HARMONY import { checkInstall } from "@/uni_modules/jkr-abc-epay" //
            #endif // 检查农行APP是否安装 const isInstalled = checkInstall()</text
          >
        </view>
      </view>
      ...
      <view class="code-card">
        ...
        <view class="code-block">
          <text class="code-text"
            >// #ifdef APP-HARMONY import { callPay } from "@/uni_modules/jkr-abc-epay" // #endif
            callPay({ url: "http://example.com/pay?TOKEN=YOUR_TOKEN", isRelease: false, success:
            (res) => { console.log("支付成功", res.message) }, fail: (err) => {
            console.error("支付失败", err.errMsg) } })</text
          >
        </view>
      </view>
      ...
      <view class="code-card">
        ...
        <view class="code-block">
          <text class="code-text"
            >// #ifdef APP-HARMONY import { startBankABC } from "@/uni_modules/jkr-abc-epay" //
            #endif startBankABC({ method: "pay", token: "1111111111111", isRelease: false, success:
            (res) => { console.log("支付成功", res.message) }, fail: (err) => {
            console.error("支付失败", err.errMsg) } })</text
          >
        </view>
      ...
      <view class="notice-card">
        ...
        <view class="notice-item">
          ...
          <text class="notice-text"
            >需要在 entry 模块的 module.json5 中配置 querySchemes: ["bankabc"]</text
          >
        </view>
        <view class="notice-item">
          <u-icon name="info-circle" size="32" color="#ff9900"></u-icon>
